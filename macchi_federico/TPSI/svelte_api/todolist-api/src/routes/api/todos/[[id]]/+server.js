...
export async function GET({ params, request, url }) {
    ...
    try {
        // se c'è id → prendo uno specifico
        if (params.id) {
        ...
        if (url.searchParams.has("done")) {
            return exec_query(
                sql_azione4,
                JSON.parse(url.searchParams.get("done"))
            );
        }

        // altrimenti prendo tutto
...
export async function PUT({ params, request }) {
  try {
    ...
    console.log("Ricevuto request HTTP PUT con parametro:", params);
